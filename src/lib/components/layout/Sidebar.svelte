	...
	const navItems: NavItem[] = [
		...
		},
		{
			...
			children: [
				...
				{ label: 'Timetable Ruang', href: '/ruang-kelas/timetable', icon: '' }
			]
		},
		{
			label: 'Nilai & KRS',
			...
			children: [
				...
				{ label: 'Transkrip', href: '/transkrip', icon: '' }
			]
		}
	];
...
<aside class="app-sidebar flex min-h-screen w-64 flex-col border-r" aria-label="Navigasi sidebar">
	<div class="p-4 border-b border-base-300/50">
		<a href="/" class="flex items-center gap-3 group">
			<div class="brand-mark w-9 h-9 rounded-lg flex items-center justify-center font-display font-semibold text-lg transition-transform group-hover:scale-105">
				U
			</div>
			{#if !collapsed}
				<div>
					<div class="font-display font-semibold text-sm tracking-tight">UMD Akademik</div>
					<div class="text-xs text-subtle">Sistem Informasi</div>
				</div>
			{/if}
		</a>
	</div>

	<nav class="flex-1 p-3 space-y-0.5 overflow-y-auto" aria-label="Menu utama">
		{#each navItems as item}
			{#if item.children}
				<div class="py-1">
					...
					class="nav-item w-full {expandedItems.has(item.label) || hasActiveChild(item) ? 'text-base-content' : ''}"
						aria-expanded={expandedItems.has(item.label) || hasActiveChild(item)}
						aria-controls={groupId(item.label)}
					>
					<span class="text-current-muted">{@html item.icon}</span>
					{#if !collapsed}
						<span class="flex-1 text-left text-sm">{item.label}</span>
					<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-current-subtle transition-transform duration-200 {expandedItems.has(item.label) || hasActiveChild(item) ? 'rotate-180' : ''}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5" aria-hidden="true">
	...
	{#if !collapsed}
		<div class="p-4 border-t border-base-300/50">
			<div class="sidebar-footer text-center">
				<div class="font-medium text-muted">Universitas Merdeka Digital</div>
				<div class="text-subtle mt-0.5">© {currentYear}</div>
			</div>
		</div>
	{/if}
</aside>
