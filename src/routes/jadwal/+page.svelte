<script lang="ts">
	...
	async function handleDelete(item: Jadwal) {
		if (!confirm(`Hapus jadwal ini? Tindakan ini tidak bisa dibatalkan.`)) return;
		
		deleteLoading = item.id;
		try {
			const response = await jadwalService.delete(item.id);
			if (response.success) {
				loadData();
			} else {
...
<div class="space-y-4">
	<div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
		<div>
			...
			<p class="text-muted">Kelola data jadwal perkuliahan</p>
		</div>
